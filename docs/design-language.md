> **Отложено. В коде этого нет.**
>
> «Relief» откатили коммитом `ec5ba80`: дерево вернулось в состояние до редизайна,
> и приложение снова живёт на скинах (`src/theme/skins.ts`). Страница оставлена
> как есть, потому что работа сделана и к идее можно вернуться — вся она лежит
> в ветках `relief`, `relief-tokens-wip` и `redesign`.
>
> Читая дальше, держи в голове: это замысел, а не описание того, что работает.
